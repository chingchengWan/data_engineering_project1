...
file = open('/Users/wan/segmentation_of_word/ettoday.rec','r', encoding = 'utf-8', errors = 'ignore') #開啟檔案,須以r讀模式
paragraph = file.readlines()

new_sents = [] #空列表用来儲存body內容
bodyfile = sys.stdout #儲存原始的輸出對象
sys.stdout = open('body_file.txt','w', encoding = 'utf-8') #重定向所有的寫入内容到 bodyfile.txt 文件
